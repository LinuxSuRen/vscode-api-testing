...
const atestCodeLense = require('./api-testing.js');
const atestCodeLenseMock = require('./api-testing-mock.js')
	...
	const toggleLink = {
		provideCodeLenses: function (document, token) {
			if (document.lineAt(0).text === "#!api-testing") {
				return atestCodeLense(document, token)
			} else if (document.lineAt(0).text === "#!api-testing-mock") {
				return atestCodeLenseMock.codeLense(document, token)
			}
		},
